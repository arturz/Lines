...
import { CarrotPowerUp, MapCellsSizes } from "../../types";
...
const PowerUps: React.FC<ComponentProps> = ({ cellPx, offset, powerUps }) => {
  const elements = [];
  ...
  for (let i = 0; i < powerUps.length; i++) {
    ...
    switch (powerUp.type) {
      case "CARROT":
        const x = elements.push(
          <G
            key={i}
            x={
              (powerUp as CarrotPowerUp).x * cellPx +
              offset.width -
              Sizes.CARROT_POWER_UP / 2
            }
            y={
              (powerUp as CarrotPowerUp).y * cellPx +
              offset.height -
              Sizes.CARROT_POWER_UP / 2
            }
          >
            <Svg
              viewBox="0 0 512 512"
              height={Sizes.CARROT_POWER_UP}
              width={Sizes.CARROT_POWER_UP}
            >
              <Path
                fill="green"
